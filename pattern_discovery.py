...
class PatternDiscovery:
    ...
    def _discover_sequential_patterns(self, events_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            # Note: pattern_insights removed - now generated by LLM synthesis
        }
    ...
    def _rule_based_clustering(self, features_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Analyze each cluster (no hardcoded labels - let LLM interpret)
        for cluster_id in unique_clusters:
            ...
            # Use neutral labels - LLM will create personas
            if cluster_id == -1:
                label = 'outliers'
            else:
            ...
            segments[label] = {
                ...
                # Note: description removed - now generated by LLM synthesis
            }

        # Note: Removed hardcoded fallback segments (strugglers, quick_bookers)
        # LLM will interpret cluster characteristics and create personas
    ...
    def _detect_friction_points(self, events_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            # Note: friction_summary removed - now generated by LLM synthesis
        }
    ...
    def _mine_association_rules(self, events_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Define success events (booking funnel completion)
        success_events = {
            'payment_success', 'booking_confirmed', 'payment_initiate',
            'ticket_confirmed', 'booking_complete', 'payment_completed'
        }

        # Onboarding events (exclude from dropout analysis)
        onboarding_events = {
            'app_start', 'choose_language', 'Onboarding_Language',
            'generate_otp', 'Onboarding_get_otp', 'get_otp', 'enter_otp',
            'verify_otp', 'Onboarding_signup_', 'Onboarding_Profile Saved',
            'App Installed', 'DeviceInfo', 'Onboarding_backpressed'
        }

        for session_id, session_df in events_df.groupby('session_id'):
            event_counts = session_df['event_name'].value_counts().to_dict()
            session_events = set(session_df['event_name'].unique())

            # Skip pure onboarding sessions
            if session_events.issubset(onboarding_events):
                continue

            # Define outcome based on reaching key booking events
            is_successful = bool(session_events & success_events)

            # Alternative: reached seat selection without dropping off
            if not is_successful and 'select_seat' in session_events:
                # If they reached seat selection and didn't abandon, it's partial success
                is_successful = 'booking_page_dropoff' not in session_events

            session_outcomes[session_id] = is_successful

            # Create transaction with repeated events marked
            # EXCLUDE onboarding events from the transaction
            transaction = []
            for event, count in event_counts.items():
                # Skip onboarding events
    ...
    def synthesize_with_llm(self, patterns: Dict, business_context: str = "bus ticket booking app") -> Dict:
        """
        Apply LLM synthesis layer to generate contextual insights.

        This replaces hardcoded if/else interpretation with adaptive LLM intelligence.

        Args:
            patterns: Raw patterns from discover()
            business_context: Business domain for contextual interpretation

        Returns:
            Enhanced patterns with LLM-generated insights
        """
        try:
            from pattern_synthesis import PatternSynthesizer

            synthesizer = PatternSynthesizer(business_context=business_context)
            synthesized_patterns = synthesizer.synthesize_all_patterns(patterns)

            return synthesized_patterns
        except Exception as e:
            print(f"\n⚠️  LLM synthesis failed: {str(e)}")
            print("   Falling back to raw patterns without LLM insights")
            return patterns
